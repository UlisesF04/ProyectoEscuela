...
import { FiUsers, FiBookOpen, FiUserCheck, FiCalendar } from 'react-icons/fi';
...
function CoursesSection() {
  ...
  return (
    <Box>
      <Heading size="lg" mb={6}>Cursos</Heading>
      <Button
        leftIcon={<AddIcon />}
        colorScheme="blue"
        mb={4}
...
function StudentsSection() {
  ...
  useEffect(() => { fetchStudents(); fetchAllCourses(); }, []);
  ...
  const studentActions = (s) => {
    if (s.is_active === false) {
      return [
        ...
        {
          label: 'Eliminar', colorScheme: 'red', variant: 'solid',
          onClick: (student) => openDeleteConfirm(student),
        },
  ...
  return (
    <Box>
      <Heading size="lg" mb={6}>Alumnos</Heading>
      <Button
        leftIcon={<AddIcon />}
        colorScheme="blue"
        mb={4}
...
function TeachersSection() {
  ...
  const openTeacherModal = async (teacher) => {
    ...
    try {
      ...
      const subjectPromises = (courses || []).map((c) => adminService.getSubjects(c.id));
      ...
      const checkPromises = allSubjects.map((s) =>
        adminService.getTeachers(s.id).then((teachers) => ({
          subject: s,
          course: courses.find((c) => c.id === s.course_id),
          isAssigned: teachers.some((t) => t.user_id === teacher.id),
        }))
      );
...
export default function AdminDashboard() {
  const sections = [
    { id: 'users', label: 'Usuarios', icon: FiUsers, component: UsersSection },
    { id: 'courses', label: 'Cursos', icon: FiBookOpen, component: CoursesSection },
    { id: 'students', label: 'Alumnos', icon: FiUserCheck, component: StudentsSection },
    { id: 'teachers', label: 'Docentes', icon: FiCalendar, component: TeachersSection },
  ];
