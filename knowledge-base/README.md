...
El proyecto **"Optimización de la Gestión Académica y Comunicación Escolar"** es una plataforma web integral para escuelas secundarias que centraliza la gestión de asistencias, calificaciones y tareas, y agrega un agente automatizado que notifica vía WhatsApp a las familias ante eventos críticos (faltas, notas bajas, tareas próximas a vencer). Está compuesto por un frontend React + Chakra UI (Vercel), un backend Node.js + Express + Sequelize con PostgreSQL (Railway), y un agente Python + Twilio (Railway Worker). Cuenta con 5 actores, 22 funcionalidades, 20 reglas de negocio y una arquitectura en 5 capas con patrones Repository, Service Layer y Middleware Chain.
